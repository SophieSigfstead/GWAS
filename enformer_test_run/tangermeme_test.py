...
def main():
    ...
    # BELOW is just to ensure that the wrapped model outputs the same dims as the original model
    #pred = model(one_hot)
    #print(pred['human'].shape)
    #del pred
    ...
    with torch.cuda.amp.autocast(): # this is supposed to reduce
        X_attr = deep_lift_shap(wrapped_model, X, target=200, device = 'cpu', random_state=0, batch_size = 4, references = reference_seq, verbose = True)
