...
def match_csv_txt(gwas_directory, directory_1000_genomes, track_list):
    chunksize = 100000
    output_path = './gwas_3_scz_matching'
    clear_directory(output_path)

    # Load text file data into memory and filter relevant columns
    text_data = pd.read_csv(gwas_directory, sep='\t', names = ['CHR','SNP','BP','A1','A2','FRQ_A_35476','FRQ_U_46839','INFO','OR','SE','P','ngt','Direction','HetISqt','HetChiSq','HetDf','HetPVa'])
    text_data = text_data[['CHR', "BP", 'SNP',  'A1' ,'A2' ,'FRQ_A_35476','FRQ_U_46839' , 'OR', 'SE', 'P']]
    ...
    for i, csv_file in enumerate(csv_files):
        ...
        for chunk_index, chunk in enumerate(pd.read_csv(csv_file_path, chunksize=chunksize)):
            ...
            merged_chunk = chunk.merge(text_data, how='left', left_on=['chr', 'pos'], right_on=['CHR', "BP"])
            ...
            for track in track_list:
                ...
                chunk_filtered_sad = merged_chunk[[sad_column] + csv_columns_no_sad + ["CHR", "BP", 'SNP',  'A1' ,'A2' ,'FRQ_A_35476','FRQ_U_46839' , 'OR', 'SE', 'P']].reset_index(drop=True)
    ...
    for track in track_list:
        ...
        if not remaining_text_data_with_nulls.empty:
            remaining_text_data_with_nulls = remaining_text_data_with_nulls[[f'SAD{track}'] + csv_columns_no_sad + ["CHR", "BP", 'SNP',  'A1' ,'A2' ,'FRQ_A_35476','FRQ_U_46839', 'OR', 'SE', 'P']]
